...
technical/plos
```
Find is used to search through the directory, technical, for all directories and list out their paths. This is useful when looking for or listing specific directories.
```
...
```
-empty combined with -type d looks for empty directories inside the directory, technical. This is useful for finding directories that have no files in them. 
```
